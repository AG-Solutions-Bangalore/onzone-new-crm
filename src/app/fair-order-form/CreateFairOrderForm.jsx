...
const CreateFairOrderForm = () => {
  ...
  const retailerOptions = (
    Array.isArray(retailerData) ? retailerData : retailerData?.data || []
  ).map((ret) => ({
    label: ret.retailer_name || ret.name,
    value: ret.retailer_name || ret.name,
    gst: ret.retailer_gst_no || ret.gst_no || ret.gst || "",
    mobile: ret.retailer_mobile || ret.mobile || "",
  }));
  ...
  const handleRetailerSelect = (selected) => {
    if (selected) {
      setFormData((prev) => ({
        ...
        fair_order_retailer_mobile:
          selected.mobile || prev.fair_order_retailer_mobile,
      }));
    }
  };
              ...
              <div className="space-y-2">
                ...
                {retailerOptions.length > 0 ? (
                  <Select
                    ...
                    placeholder="Select or search retailer..."
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <div className="inline-flex rounded-lg p-1 bg-gray-100 border border-gray-200">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${entryMode === "typing"
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md transition-colors ${entryMode === "scanner"
                        ? "bg-white text-orange-600 shadow-xs"
                        : "text-gray-600 hover:text-gray-900"
                      }`}
                  >
              ...
              <div className="space-y-3 pt-2">
                {subItems.length > 0 ? (
                  ...
                  <div className="text-center py-8 border-2 border-dashed rounded-lg text-gray-400">
                    <p className="text-sm">
                      No items added yet. Enter or scan barcode above to select sizes.
                    </p>
                  </div>
                )}
              </div>
            ...
            {ALL_SIZES.map((sz) => {
              ...
              return (
                <button
                  ...
                  className={`py-3 rounded-2xl text-base font-black transition-all duration-150 flex items-center justify-center select-none ${isSelected
