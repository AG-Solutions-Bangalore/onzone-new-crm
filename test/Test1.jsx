...
const formSchema = z.object({
  ...
  workorder_sub_data: z.array(
    z.object({
      ...
      work_order_sub_a: z.string(),
      work_order_sub_b: z.string(),
      work_order_sub_c: z.string(),
      work_order_sub_length: z.string(),
      work_order_sub_new_length: z.string(),
      work_order_sub_half_shirt: z.string(),
      work_order_sub_full_shirt: z.string(),
      work_order_sub_amount: z.string(),
    })
  ),
  ...
  const onSubmit = async (e) => {
    ...
    if (!validation.success) {
      const errorMessages = validation.error.errors.map((e) => {
        const fieldLabel = e.path[0].replace(/_/g, ' '); // Replace underscores with spaces for better readability
        return `${fieldLabel.charAt(0).toUpperCase() + fieldLabel.slice(1)}: ${e.message}`;
      }).join(", ");
